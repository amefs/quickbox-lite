...
function _update_version() {
    ...
    _run sed -i -E "s|^([[:space:]]*\"version\"[[:space:]]*:[[:space:]]*\")[0-9]+\.[0-9]+\.[0-9]+(\"[[:space:]]*,?[[:space:]]*)$|\1${VERSION}\2|" setup/dashboard/backend/package.json
    ...
    _run git add setup/dashboard/backend/package.json
}
