...
describe("router — HTTP routes", () => {
    ...
    describe("GET /", () => {
        ...
        it("should render the dashboard shell from browser locale cookie", async () => {
        ...
        it("should render the dashboard shell from Accept-Language when no explicit locale is set", async () => {
            const res = await request(app)
                .get("/")
        ...
        it("should render the dashboard shell from a browser locale region without a base fallback", async () => {
            const res = await request(app)
                .get("/")
        ...
        it("should preserve the legacy browser script order", async () => {
            ...
            for (const script of orderedScripts) {
                const currentIndex = res.text.indexOf(script, previousIndex + 1);
    ...
    describe("static dashboard assets", () => {
        const assetPaths = [
            "/skins/quick.css",
            "/lib/jquery/jquery.min.js",
        ...
        for (const assetPath of assetPaths) {
            it(`should serve ${assetPath}`, async () => {
                ...
                expect(res.status).to.equal(200);
                expect(byteLength || Number(res.headers["content-length"]) || 0).to.be.greaterThan(0);
            });
        }
    });
